...
static bool move_matches(const std::string &alg, const Move &m)
{
    if (alg.size() < 4)
        return false;
...
static std::vector<std::string> parse_bm(const std::string &bm_field)
{
...
void run_benchmark(const std::string &epd_path, int depth)
{
    ...
    if (!file.is_open())
    {
        std::cerr << "Could not open puzzle file: " << epd_path << "\n";
        return;
    }
    ...
    while (std::getline(file, line))
    {
        ...
        std::string bm_rest = line.substr(bm_pos + 4);

        std::vector<std::string> best_moves = parse_bm(bm_rest);
        if (best_moves.empty())
            continue;
        ...
        //auto t0 = std::chrono::steady_clock::now();
        // tried using iterative deeping with time limit
        ...
        {
            if (played == bm || move_matches(bm, result.best_move))
            {
                found = true;
                break;
            }
